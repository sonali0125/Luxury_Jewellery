        ...
        {mangalsutraData.map((m) => (
          <div key={m.id} className="card">
            <div className="imageBox">
              ...
              <button className="quickViewBtn" onClick={() => openModal(m)}>Quick View</button>
            </div>
            <div className="cardInfo">
              <h4>{m.name}</h4>
              <p className="priceText">{m.price}</p>
              <button className="viewDetailsBtn" onClick={() => openModal(m)}>View Details →</button>
            </div>
          </div>
        ))}
      </main>
            ...
            {acquisitionStep === 'form' && (
              <div className="acquisitionFormContainer">
                ...
                <form onSubmit={handleAcquisitionSubmit}>
                  ...
                  <input disabled value={`Reference: MS-0${selectedItem.id}`} />
